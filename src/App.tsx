...
import { ModeratorBoard,AdminBoard,CurrentUser,UserProfile,LogIn,SignUp } from "./components/general.components";
...
const App:React.FC = ():React.ReactElement => {
  ...
  const logOut = ():void =>{
    logoutUser();
    setState(prevState =>{
      const _user = { showModeratorBoard: false,
                      showAdminBoard: false,
                      currentUser: undefined};
  ...
  return (
    <div>
      <nav className="navbar navbar-expand navbar-dark bg-dark">
        <Link to={"/"} className="navbar-brand"> Sundstrom </Link>
        ...
        {state.currentUser ? ( <UserProfile username={state.currentUser.username ?? ""} callback={logOut}/>) : 
        (<div className="navbar-nav ml-auto"><LogIn/><SignUp/></div>)}
      </nav>
